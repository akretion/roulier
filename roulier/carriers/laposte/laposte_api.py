...
class LaposteApi(Api):
    def _service(self):
        ...
        schema['labelFormat']['allowed'] = list(LAPOSTE_LABEL_FORMAT) + ["PDF", "ZPL", "DPL"]
        ...
        schema['returnTypeChoice'] = {'default': '', 'description': """Obligatoire pour certains colis à l’international, selon les zones tarifaires applicables. Indique si le colis doit être retourné à l’expéditeur en cas de non distribution du colis"""}
        schema['returnType'] = {'default': '', 'description': """Utilisé pour le Colissimo Retour uniquement. Définit le mode de transmission de l’étiquette"""}
        return schema
    ...
    def _address(self):
        ...
        schema['street1'].update({'required': False, 'empty': True, 'description': 'Entrée, bâtiment, immeuble, résidence'})
        schema['street0'] = {'default': '', 'description': 'Etage, couloir, escalier, appart.'}
        schema['street3'] = {'default': '', 'description': """Lieu dit ou autre mention"""}
    ...
    def _to_address(self):
        ...
        schema['firstName'] = {'default': '', 'description': """Prénom. Obligatoire pour So Colissimo"""}
        schema['street2'].update({'required': True, 'description': """Numéro et libellé de voie. Ex : « 5 rue du Bellay »"""})
        schema['street1'].update({'required': False, 'empty': True, 'description': 'Entrée, bâtiment, immeuble, résidence'})
        return schema
    ...
    def _parcel(self):
        ...
        schema['codAmount'] = {'default': '0', 'description': """Montant attendu lors de la livraison contre remboursement. Par défaut, renseigner « 0 » (zéro)"""}
        schema['ftd'] = {'default': '0', 'description': """Pour les envois vers l’Outre-Mer uniquement Indique si le colis est franc de taxes et de droits. Par défaut, renseigner « 0 » (zéro)"""}
        return schema
